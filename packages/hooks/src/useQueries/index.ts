 ...
 * @updated      : 2022/04/20 00:38:37 +0800
 * @description  : ****
 */
...
type SetQueriesAction<Q> = ((prevQueries: Q) => Q);
...
export function useQueriesCore<Q>(initialQueries?: InitialQueries<Q>, navi?: NavigatorAction): [Q, DispatchQueries<Q | SetQueriesAction<Q>>] {
  ...
  const dispatch = useCallback((nextQueries: Q | SetQueriesAction<Q>): void => {
    setQueries((searches: Q): Q => {
      if (typeof nextQueries === 'function') {
        return toQueriesObject(toSearchString((nextQueries as SetQueriesAction<Q>)(searches)));
      }

      return toQueriesObject(toSearchString(nextQueries as Q));
    });
  }, [ queries ]);
